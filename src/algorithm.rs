// mod geometry;
use crate::geometry::Point as Point;
use crate::geometry::Edge as Edge;
use crate::geometry::Triangle as Triangle;
use crate::geometry::SpacePartition as SpacePartition;
...
pub fn de_wall(P: Vec<Point>, sp: &SpacePartition, afl: &mut Vec<Edge>) -> Vec<Triangle> {
    ...
    plot_step(step_counter, &sp.alpha, &P1, &P2, &afl_1, &afl_2, &afl_sigma, &sigma);
    step_counter += 1;
    ...
    while !afl_sigma.is_empty() {
        ...
        println!("Looking at face ({} {}), ({} {})", f.start.x, f.start.y, f.end.x, f.end.y);
    ...
    //TODO: Check, move to new fn with test
    let (sp1, sp2) = sp.partition(); 
    if !afl_1.is_empty(){
        sigma.append(&mut de_wall(P1, &sp1, &mut afl_1));
    }
    if !afl_2.is_empty(){
        sigma.append(&mut de_wall(P2, &sp2, &mut afl_2));
    }
...
fn make_simplex(step_counter: i32, f: &Edge, P: &Vec<Point>) -> Option<Triangle> {
  ...
  'outer: for p in &filtered_P {
    ...
    println!("Checking point {} {} with delauney distance {}", p.x, p.y, dd(f, p));
...
fn plot_step(step_counter :i32, alpha: &Edge, P1: &Vec<Point>, P2: &Vec<Point>,
            afl_1: &Vec<Edge>, afl_2: &Vec<Edge>, afl_sigma: &Vec<Edge>, sigma: &Vec<Triangle> ) {
    let imgx = 800;
    let imgy = 600;
    ...
    let steps = 500;
    for n in 0..steps {
        let x = alpha.start.x as i32 + ((n as f32 / steps as f32) * (alpha.end.x as f32 - alpha.start.x as f32))as i32;
        let y = alpha.start.y as i32 + ((n as f32 / steps as f32) * (alpha.end.y as f32 - alpha.start.y as f32))as i32;
